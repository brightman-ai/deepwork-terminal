...
一个**移动端优先的 web 终端，用来盯住和操控你的 AI 编码 agent**（Claude Code / Codex）——从任何地方够到 agent 真正运行的那台机器：路上用**手机**，或在远程机器上用**一个浏览器标签页代替 SSH 会话**。内置鉴权、一个 flag 开公网的 Cloudflare 隧道、Web Push / 微信通知，以及嵌入式 Vue 前端。一行 `http.Handler` 即可挂进任意 Go 应用。
...
而且它不只在你离开键盘时有用。**SSH 到一台机器上去跑 agent？** 一个浏览器标签页，在那些 SSH 下别扭的事上都更顺手：不用记快捷键就能切 tmux pane、几个 agent 里**哪一个**跑完了立刻收到提醒、把文件拖上去或拉下来、把截图直接粘给 agent——不用 `scp`，不用翻找是哪个 pane。

...
export function livenessCopy(
  ...
  return {
    ...
    // 同样刻意不说死原因（见 reopenDetached.ts 里 reopenNoticeLine 的注释）：自从 PTY 归常驻
    // daemon 持有，"服务重启"已经不再是进程结束的常见原因，最常见的反而是用户自己敲了 `exit`
    // 或者程序崩了。断言一个我们并不知道的死因，就是在一张专门用来讲实话的卡片上讲假话。
    body: ctx.remote
      ? `${where} 上原来这个终端的进程已经不在了。里面运行的程序没有在后台继续跑，之前的输出也没有留下来。`
      : '原来在这个终端里运行的程序已经结束了。它没有在后台继续跑，之前的输出也没有留下来。',
